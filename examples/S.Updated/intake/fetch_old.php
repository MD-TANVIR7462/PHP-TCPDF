...
if(isset($_GET['formNo'])){
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if($id==""){
			...
			$result = $objQuery->inUpDel("UPDATE intake_form_info SET note = '$note' WHERE id='$id'");			
		}
		if($result){
			$clientId = $objQuery->indexByQuerySingleData("SELECT * from client_info WHERE sha1(MD5(id))='$_POST[client_id]'")->id;
			$objClientFile->clientProfileUpdate($clientId,$_POST);
			echo 'Information Successfully Updated';
		} else {
			echo "Failed to update.UPDATE intake_form_info SET note = '' WHERE id='$id'" ;
		}
	}
} else {
